...
def parse_stations_from_dir(stations_dir):
    """
    지상국 설정 파일(.txt / .cfg)을 읽어 튜플 리스트로 반환합니다.

    [파일 포맷 - 하위 호환 유지]
    Station_Name, Latitude, Longitude, Is_Download(Y/N), Is_Command(Y/N), [RX_Capacity], [TX_Capacity]

    - 앞의 5개 컬럼(이름/위도/경도/다운로드가능/커맨딩가능)은 기존과 동일합니다.
    - 뒤에 RX_Capacity(동시 수신 가능 대수), TX_Capacity(동시 커맨딩 가능 대수) 2개 컬럼을
      "선택적으로" 추가할 수 있습니다. 기존 파일처럼 이 컬럼이 없으면 각각 기본값 1로 처리되어,
      기존에 작성해둔 지상국 파일도 수정 없이 그대로 동작합니다(하위 호환).
    - RX_Capacity: 이 지상국에 안테나가 여러 대 있어 동시에 여러 위성을 수신할 수 있는 경우
      해당 대수를 입력합니다 (예: 안테나 3대 -> 3).
    - TX_Capacity: 보통 업링크(커맨딩) 체인은 1개뿐이므로 기본값 1을 그대로 쓰면 되고,
      특수하게 업링크 체인이 여러 개인 지상국만 값을 올려주면 됩니다.

    반환되는 튜플의 인덱스는 다음과 같습니다 (기존 코드와의 호환을 위해 순서 고정):
      0: name, 1: lat, 2: lon, 3: is_download(Y/N), 4: is_command(Y/N),
      5: rx_capacity(int, 기본 1), 6: tx_capacity(int, 기본 1)
    """
    stations_list = []
    if not os.path.exists(stations_dir):
        ...
        with open(default_file, "w", encoding="utf-8") as f:
            f.write("# Station_Name, Latitude, Longitude, Is_Download_Capable(Y/N), Is_Command_Capable(Y/N), RX_Capacity(optional), TX_Capacity(optional)\n")
            f.write("# RX/TX_Capacity를 생략하면 자동으로 1(안테나 1대)로 간주됩니다.\n")
    ...
    def _parse_capacity(raw_value, default_value=1):
        """RX/TX Capacity 컬럼을 안전하게 정수로 변환. 값이 없거나 이상하면 기본값(1)으로 보정."""
        try:
            val = int(float(raw_value))
            return val if val >= 1 else default_value  # 0 이하 값은 "슬롯 없음" 버그를 막기 위해 1로 보정
                        ...
                        for line in f:
                            ...
                            if len(parts) >= 3:
                                try:
                                    ...
                                    # 💡 신규: RX/TX 안테나 동시 처리 대수 (없으면 기본값 1 = 기존 동작과 동일)
